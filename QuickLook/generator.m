...
OSStatus GBQuickLookRender(CGContextRef cgContext, CFURLRef url, bool showBorder)
{
    /* Load the template UIImages when generating the first thumbnail */
    static UIImage *template = nil;
    static UIImage *templateUniversal = nil;
    static UIImage *templateColor = nil;
    static NSBundle *bundle = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        bundle = [NSBundle bundleForClass:NSClassFromString(@"GBPanel")];
    ...
    /* Convert the screenshot to a magnified NSImage */
    UIImage *screenshot = [[UIImage alloc] initWithCGImage:iref];
    /* Draw the screenshot */
    if (showBorder) {
