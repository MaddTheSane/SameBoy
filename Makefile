...
$(OBJ)/HexFiend/%.m.o: HexFiend/%.m
	-@$(MKDIR) -p $(dir $@)
	$(CC) $(CFLAGS) $(FRONTEND_CFLAGS) $(FAT_FLAGS) $(OCFLAGS) -c $< -o $@ -fobjc-arc -include HexFiend/HFFrameworkPrefix.h
...
$(BIN)/SameBoy-iOS.app: $(BIN)/SameBoy-iOS.app/SameBoy \
                        ...
                        $(shell ls AppleCommon/SharedAssets.xcassets/*.imageset/*.json) \
                        $(shell ls AppleCommon/SharedAssets.xcassets/*.imageset/*.png) \
                        $(shell ls AppleCommon/SharedAssets.xcassets/*.appiconset/*.json) \
                        $(shell ls AppleCommon/SharedAssets.xcassets/*.appiconset/*.png) \
                        $(shell ls AppleCommon/AppIcon.icon/*.json) \
                        $(shell ls AppleCommon/AppIcon.icon/Assets/*.png) \
...
$(BIN)/SameBoy-iOS.app/SameBoy: $(CORE_OBJECTS) $(IOS_OBJECTS)
	-@$(MKDIR) -p $(dir $@)
	$(CC) $^ -o $@ $(LDFLAGS) -weak_framework UniformTypeIdentifiers
ifeq ($(CONF), release)
	$(STRIP) $@
endif
